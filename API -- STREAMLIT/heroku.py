...
st.set_page_config(layout='wide')
   #aumenta a proporção dos caracteres na pagiuna web

@st.cache(allow_output_mutation=True)

def get_data(path):
   ### SEMPRE por : no final de uma funçãos
...
def overview_data(data):
   ...
   if (f_zipcode!=[]) :
       ...
       # isin() filtra o dataframe de acordo com linhas especificas
   else:
       data = data.copy()
   ...
   df1.columns = ['attributes', 'max_', 'min_', 'media', 'mediana', 'std']
...
def commercial_distribution(data):
   ...
   f_date = st.sidebar.slider('Date', max_date, min_date)
